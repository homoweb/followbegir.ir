...
test('admins can list, create, toggle and delete users', function () {
    $this->get('https://admin.followbegir.test/users')
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
            ->component('Admin/Users/Index')
            ->has('users.data', 1));

    $this->post('https://admin.followbegir.test/users', [
        'name' => 'کاربر تازه',
    ...
    $this->patch('https://admin.followbegir.test/users/'.$target->id.'/toggle')
        ->assertRedirect()->assertSessionHas('success');
    expect($target->refresh()->is_active)->toBeFalse();

    // An admin can never deactivate or delete their own account.
    $this->patch('https://admin.followbegir.test/users/'.$this->admin->id.'/toggle')
        ->assertSessionHas('error');
    expect($this->admin->refresh()->is_active)->toBeTrue();

    $this->delete('https://admin.followbegir.test/users/'.$target->id)
        ->assertSessionHas('success');
    expect(User::query()->find($target->id))->toBeNull();

    $this->delete('https://admin.followbegir.test/users/'.$this->admin->id)
        ->assertSessionHas('error');
...
test('admins can update users including optional password changes', function () {
    ...
    $this->put('https://admin.followbegir.test/users/'.$target->id, [
        'name' => 'نام تازه',
...
test('admins can manage products and their price tiers', function () {
    ...
    $this->post('https://admin.followbegir.test/products', $payload)
        ->assertRedirect()->assertSessionHas('success');
    ...
    $this->put('https://admin.followbegir.test/products/'.$product->id, $payload)
        ->assertRedirect()->assertSessionHas('success');
    ...
    $this->patch('https://admin.followbegir.test/products/'.$product->id.'/toggle')
        ->assertSessionHas('success');
    ...
    $this->delete('https://admin.followbegir.test/products/'.$product->id)
    ...
    $this->delete('https://admin.followbegir.test/products/'.$plain->id)
        ->assertSessionHas('success');
...
test('admins can advance order status', function () {
    ...
    $this->patch('https://admin.followbegir.test/orders/'.$order->id.'/status', [
        'status' => 'completed',
    ])->assertRedirect()->assertSessionHas('success');
...
test('non-admins and guests are kept out of the admin panel', function () {
    ...
    $this->actingAs($user)
        ->get('https://admin.followbegir.test/users')
        ->assertForbidden();

    // The login route is guest-only, so drop the session's user first.
    $this->app['auth']->guard()->logout();

    $this->post('https://admin.followbegir.test/login', [
        'email' => $user->email,
        'password' => 'password',
    ])->assertSessionHasErrors(['email']);
    $this->assertGuest();

    $response = $this->get('https://admin.followbegir.test/users');
